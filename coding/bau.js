...
async function combineFromProject(project){
	...
	files = files.filter(x=>!x.startsWith('start') && !x.startsWith('test') && !x.startsWith('app.'));
...
function generateCodeText(cvckeys,funckeys,byKeyMinimized){
	let closuretext = '';
	for (const k of cvckeys) { closuretext += byKeyMinimized[k].code + '\n'; }
	for (const k of funckeys) { 
		closuretext += byKeyMinimized[k].code + '\n'; 
...
async function getCodeKeys(path){
	let funclist = await codeParseFile(path);
	return funclist.map(x=>x.key);
}
async function getCodeList(path){
	let funclist = await codeParseFile(path);
	return funclist;
}
