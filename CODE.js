...
async function showColors() {
	...
	let dColors = mDom(dParent, { wmax: (sz + 4) * 15, hmargin: 20, hpadding: 0, display: 'flex', gap: '2px 4px', wrap: true }, { id: 'dColors' });
	let grays = []; for (const x of '0123456789abcde') { grays.push(`#${x}${x}${x}${x}${x}${x}`) };
	list = M.playerColors.concat(grays);
	let itemsColor = DA.itemsColor = [];
	for (const c of list) {
		let dc = mDom(dColors, { w: sz, h: sz, bg: c, fg: idealTextColor(c), cursor: 'pointer' });
		let item = { div: dc, color: c, isSelected: false };
		itemsColor.push(item);
		dc.onclick = () => onclickColor(item, itemsColor);
	}
	let dcc = mDom(dColors, { bg: 'white', border: getThemeFg(), w: sz, h: sz, fz: sz * .7, cursor: 'pointer', hpadding: 4, box: true, align: 'center' }, { html: 'X' });
	let itemc = { div: dcc, color: '', isSelected: false };
	itemsColor.push(itemc);
	dcc.onclick = () => onclickColor(itemc, itemsColor);

	let dPalette = mDom(dParent, { wmax: (sz + 4) * 15, hmargin: 20, hpadding: 0, display: 'flex', gap: '2px 4px', wrap: true }, { id: 'dPalette' });

	let dTheme = mDom(dParent, { wmax: (sz + 4) * 15, margin: 20, hpadding: 0, display: 'flex', gap: '2px 4px', wrap: true });
	list = M.textures;
	...
	let dct = mDom(dTheme, { bg: 'white', border: getThemeFg(), w: sz, h: sz, fz: sz * .7, cursor: 'pointer', hpadding: 4, box: true, align: 'center' }, { html: 'X' });
	let itemt = { div: dct, bgImage: '', bgRepeat: '', bgSize: '', blend: '', isSelected: false };
	itemsTexture.push(itemt);
	dct.onclick = () => onclickTexture(itemt, itemsTexture);

	sz = 100;
	let dBlend = mDom(dParent, { wmax: (sz + 4) * 6, margin: 20, hpadding: 0, display: 'flex', gap: '2px 4px', wrap: true });
...
function testUpdateTestButtons(dParent,styles={}) {
  ...
  // mStyle('dTitle',{hmin:24});
  mIfNotRelative(dParent);
  //addKeys({ position:'absolute',right:10,patop:4,display: 'flex', 'justify-content':'end', gap: 10 },styles);
  ...
  // let [playmodeKey,sz,bg,matop,patop] = playmode == 'human'?['person',22,'dimgray',0,2]:['robot',24,'transparent',4,0];
  // [playmodeKey='robot',sz,bg]=['robot',24,'transparent']
...
async function showTable000(table){
	...
	//await mSleep(100);
	//assertion(!DA.PROCESSING,"der ist immer noch drin!!!")
	...
	//main view
	let vid='main';
	V[vid] = await func.presentTable(dOpenTable,table,me,100);
	...
	if (playmode == 'bot') {}//await func.botMove('main');
	else await func.activate('main');

	// //mini bot view bei solo game: das gilt eigentlich nur fuer setgame!
	// if (table.options.gamemode == 'solo'){
	...
	// 	let botname=firstCondDict(table.fen.players,x=>x.playmode == 'bot');
	// 	if (isdef(botname)) {
	// 		mDom(dMini,{align:'center',fg:'white',pabottom:4},{html:botname+'bot'})
	// 		V.x = await func.presentTable(dMini,table,botname,50);
	// 		await func.botMove('x');
	// 		//console.log(V.x)
	// 	}
	// }
	//DA.PROCESSING = false;
