...
async function test53_advisors() {
	...
	for (const k of list) { //in natCards){ //of arrTake(Object.keys(natCards),20)){ //in natCards){
		...
		console.log('path', path)
		//await imgToServer(canvas,'was/img/advisor/'+path);
...
async function test51_cards(name) {
	...
	//await imgToServer(canvas,`was/${name}.png`);
	//await mSleep(1000);
...
async function test50_imgToLandscape() {
	...
	// let resp = await imgToPortrait(path, 200, 'dMain', true, `was/age1_aeneid.jpg`);	console.log('resp',resp)
...
async function YES_test12_save(){
	...
	let o = {path:'../was/test.txt',data:{text:'I am Sam',pos:22},mode:'a'};
	let resp = await uploadJson('save',o)
	console.log('response',resp);
